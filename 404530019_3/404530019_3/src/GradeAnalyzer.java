...
public class GradeAnalyzer {
	...
	boolean isValidGrade(double aGrade) { // 判別成績是否正確
	...
	void addGrade(double aGrade) { // 將成績分類
	...
	void analyzeGrades() {
		...
		for (int i = 0; i < gradelist.size(); i++) {
			// 公式出錯了, 除以個數應該放在Line 78, -2.
			variance += Math.pow(gradelist.get(i) - avg, 2) / gradelist.size();// 變異數
		}
		SD = Math.sqrt(variance);// 標準差

	}

	public String toString() {
		return "You entered " + gradelist.size()
